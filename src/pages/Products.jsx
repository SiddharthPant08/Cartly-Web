...
import { HiOutlineAdjustmentsHorizontal, HiOutlineMagnifyingGlassMinus } from 'react-icons/hi2'
...
export default function Products() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      ...
      list = list.filter(
        (p) => p.title.toLowerCase().includes(term) || p.brand.toLowerCase().includes(term) || p.category.includes(term)
      )
    }
    if (dealOnly) list = list.filter((p) => p.tags?.includes('deal'))
    if (filters.categories.length) list = list.filter((p) => filters.categories.includes(p.category))
    if (filters.priceRange) {
      list = list.filter((p) => p.price >= filters.priceRange.min && p.price <= filters.priceRange.max)
    }
  ...
  const activeCategoryName = categories.find((c) => c.id === filters.categories[0])?.name
  ...
  const clearFilters = () => {
    setFilters({ categories: [], priceRange: null, rating: null })
    setPage(1)
    ...
    <div className="container-page py-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink-900">
          {q ? `Results for "${q}"` : activeCategoryName || 'All products'}
        </h1>
        <p className="text-sm text-ink-500 mt-1">{filtered.length} products found</p>
      </div>
      ...
      <div className="grid lg:grid-cols-[240px_1fr] gap-8">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 rounded-2xl bg-white border border-ink-100 p-5 shadow-card">
            <ProductFilters filters={filters} onChange={handleFiltersChange} onClear={clearFilters} />
          </div>
        </aside>
        ...
        <div>
          ...
          <div className="flex items-center justify-between gap-3 mb-5">
            ...
            <div className="ml-auto flex items-center gap-2">
              <label htmlFor="sort" className="text-sm text-ink-500 hidden sm:inline">
                Sort by
              </label>
              <select
                id="sort"
      ...
      <Modal open={mobileFiltersOpen} onClose={() => setMobileFiltersOpen(false)} title="Filters" maxWidth="max-w-sm">
        <ProductFilters filters={filters} onChange={handleFiltersChange} onClear={clearFilters} />
