...
import { HiOutlineCreditCard, HiOutlineBanknotes, HiOutlineDevicePhoneMobile, HiCheckCircle } from 'react-icons/hi2'
...
const paymentMethods = [
  ...
  { id: 'upi', label: 'UPI', icon: HiOutlineDevicePhoneMobile },
  { id: 'cod', label: 'Cash on Delivery', icon: HiOutlineBanknotes },
]
...
export default function Checkout() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const order = await placeOrder({
        items: items.map((i) => ({ id: i.id, title: i.title, image: i.image, quantity: i.quantity, price: i.price })),
        total: totals.total,
        address: address ? `${address.name}, ${address.line1}, ${address.city}, ${address.state} ${address.pincode}` : 'N/A',
        paymentMethod: paymentMethods.find((p) => p.id === payment)?.label,
      })
      clearCart()
  ...
  return (
    <div className="container-page py-8">
      <h1 className="text-2xl font-bold text-ink-900 mb-6">Checkout</h1>

      <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
        <div className="space-y-6">
          {/* Address */}
          <div className="rounded-2xl bg-white border border-ink-100 shadow-card p-5">
            <h2 className="font-bold text-ink-900 mb-4">1. Delivery address</h2>
            <div className="space-y-3">
              {addresses.map((addr) => (
                <label
                  key={addr.id}
                  className={`flex items-start gap-3 rounded-xl border p-4 cursor-pointer transition-colors ${
                    selectedAddress === addr.id ? 'border-primary-600 bg-primary-50' : 'border-ink-200 hover:border-primary-300'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <div>
                    ...
                    <p className="text-xs text-ink-500 mt-0.5">Phone: {addr.phone}</p>
                  </div>
                </label>
              ))}
            </div>
          ...
          {/* Payment */}
          <div className="rounded-2xl bg-white border border-ink-100 shadow-card p-5">
            <h2 className="font-bold text-ink-900 mb-4">2. Payment method</h2>
            <div className="grid sm:grid-cols-3 gap-3">
              {paymentMethods.map((method) => (
                <label
                  key={method.id}
                  className={`flex flex-col items-center gap-2 rounded-xl border p-4 cursor-pointer transition-colors ${
                    payment === method.id ? 'border-primary-600 bg-primary-50' : 'border-ink-200 hover:border-primary-300'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <method.icon size={22} className={payment === method.id ? 'text-primary-600' : 'text-ink-500'} />
                  <span className="text-xs font-medium text-ink-800 text-center">{method.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* Items review */}
          <div className="rounded-2xl bg-white border border-ink-100 shadow-card p-5">
            <h2 className="font-bold text-ink-900 mb-4">3. Review items ({items.length})</h2>
            <div className="space-y-3">
              {items.map((item) => (
        ...
        <div className="rounded-2xl bg-white border border-ink-100 shadow-card p-5 sticky top-24">
          ...
          >
            {placing ? (
              ...
              <>
                <HiCheckCircle size={18} /> Place order
              </>
            )}
          </Button>
        </div>
